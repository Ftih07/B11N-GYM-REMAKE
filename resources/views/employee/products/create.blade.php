...
<head>
    ...
    <title>Tambah Produk - B1NG Empire</title>
    ...
    <nav class="bg-[#0D0D0D] p-4 shadow-lg border-b border-gray-800 sticky top-0 z-50">
        <div class="max-w-7xl mx-auto flex justify-between items-center text-white">
            <div class="font-extrabold text-xl tracking-tighter uppercase flex items-center gap-2">
                <i class="fas fa-box text-[#E31E24]"></i> Tambah Produk Baru
            </div>
        ...
        <form action="{{ route('employee.products.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                    ...
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-200">
                        <div class="mb-5 border-b border-gray-100 pb-3 flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <p class="text-[11px] text-gray-500 font-bold tracking-widest uppercase">Detail nama, deskripsi, dan varian.</p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div class="md:col-span-2">
                                ...
                                <input type="text" name="name" value="{{ old('name') }}" placeholder="Contoh: Whey Protein Isolate" required class="w-full p-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#E31E24] bg-gray-50 font-bold">
                            </div>

                            <div class="md:col-span-2">
                                <label class="text-[11px] font-bold text-gray-600 uppercase mb-1.5 block">Deskripsi <span class="text-[#E31E24]">*</span></label>
                                <textarea name="description" rows="4" placeholder="Jelaskan detail produk di sini..." required class="w-full p-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#E31E24] bg-gray-50">{{ old('description') }}</textarea>
                            </div>

                            <div>
                                <label class="text-[11px] font-bold text-gray-600 uppercase mb-1.5 block">Varian Rasa</label>
                                <input type="text" name="flavour" value="{{ old('flavour') }}" placeholder="Contoh: Coklat, Vanilla" class="w-full p-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#E31E24] bg-gray-50">
                            </div>

                            <div>
                                <label class="text-[11px] font-bold text-gray-600 uppercase mb-1.5 block">Takaran Saji</label>
                                <input type="text" name="serving_option" value="{{ old('serving_option') }}" placeholder="Contoh: 1 Scoop (30g)" class="w-full p-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#E31E24] bg-gray-50">
                            </div>
                        </div>
                    </div>

                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-200" x-data="imageViewer()">
                        <div class="mb-5 border-b border-gray-100 pb-3 flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <p class="text-[11px] text-gray-500 font-bold tracking-widest uppercase">Upload foto produk yang menarik.</p>
                            </div>
                        </div>
                        ...
                        <div class="flex items-center justify-center w-full">
                            <label class="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 border-dashed rounded-xl cursor-pointer bg-gray-50 hover:bg-gray-100 relative overflow-hidden transition">
                                <div class="flex flex-col items-center justify-center pt-5 pb-6" x-show="!imageUrl">
                                    ...
                                    <p class="mb-1 text-sm text-gray-500 font-semibold">Klik untuk upload foto</p>
                                    <p class="text-xs text-gray-400">PNG, JPG, atau WEBP (Maks 2MB)</p>
                                </div>
                                <img :src="imageUrl" x-show="imageUrl" class="absolute inset-0 w-full h-full object-contain p-2 bg-white" style="display: none;">
                ...
                <div class="lg:col-span-4 space-y-6">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" name="price" value="{{ old('price') }}" required class="w-full pl-10 p-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#E31E24] bg-gray-50 font-black">
                                </div>
                            </div>
                            <div>
                                ...
                                <select name="status" required class="w-full p-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#E31E24] bg-gray-50 font-bold text-gray-700">
                                    ...
                                    <option value="soldout" {{ old('status') == 'soldout' ? 'selected' : '' }}>🔴 Sold Out</option>
                                </select>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <select name="category_products_id" required class="w-full p-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#E31E24] bg-gray-50 font-medium">
                                    ...
                                    <option value="{{ $category->id }}" {{ old('category_products_id') == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                ...
                                <select name="stores_id" required class="w-full p-2.5 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#E31E24] bg-gray-50 font-medium">
                                    ...
                                    <option value="{{ $store->id }}" {{ old('stores_id') == $store->id ? 'selected' : '' }}>{{ $store->title }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <button type="submit" class="w-full bg-[#0D0D0D] text-white py-4 rounded-xl font-black text-lg uppercase tracking-wider hover:bg-[#E31E24] transition shadow-lg flex items-center justify-center gap-2">
