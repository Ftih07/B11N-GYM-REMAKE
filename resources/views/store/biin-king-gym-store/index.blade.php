...
<head>
    ...
    <meta name="description"
        content="Belanja kebutuhan fitness dan suplemen original dari B11N Gym dan K1NG Gym Purwokerto. Dapatkan {{ strtolower($pageTitle) }} terbaik di sini.">
    <meta name="keywords"
        content="katalog produk b1ng empire, suplemen fitness purwokerto, b11n gym, k1ng gym, {{ strtolower($pageTitle) }}">
    ...
    <meta property="og:description"
        content="Belanja kebutuhan fitness dan suplemen original dari B11N Gym dan K1NG Gym Purwokerto.">
...
<body
    class="bg-gray-50 text-brand-black dark:bg-brand-black dark:text-gray-200 font-sans antialiased transition-colors duration-300">

    {{-- TOAST NOTIFICATION --}}
    @if (session('success'))
        <div id="toast-notification"
            class="fixed top-4 right-4 z-[9999] flex items-center bg-green-600 text-white px-6 py-4 rounded-lg shadow-2xl transform transition-all duration-500 hover:scale-105">
            <i class="fas fa-check-circle text-2xl mr-3"></i>
            <span class="font-medium">{{ session('success') }}</span>
            <button onclick="document.getElementById('toast-notification').remove()"
    ...
    @if ($store)
        <header
            class="relative w-full h-[100vh] md:h-[100vh] flex items-center justify-center bg-gray-900 overflow-hidden mt-20">
            <div class="absolute inset-0 z-0">
                {{-- Optimasi: fetchpriority="high" untuk memaksa gambar Hero dimuat secepat mungkin --}}
                <img src="{{ asset('storage/' . $store->image) }}" alt="Background {{ $store->title }}"
                    class="w-full h-full object-cover opacity-60" fetchpriority="high" decoding="async">
            ...
            <div class="relative z-10 text-center px-4 max-w-4xl mx-auto" data-aos="fade-up">
                <h2 class="text-brand-red font-heading font-bold text-xl md:text-2xl tracking-[0.2em] mb-2 uppercase">
                    {{ $store->subheading }}</h2>
                <h1
                    class="text-white font-heading font-bold text-5xl md:text-7xl lg:text-8xl leading-tight mb-6 drop-shadow-lg">
                    {{ $store->title }}</h1>
                <p class="text-gray-300 text-lg md:text-xl max-w-2xl mx-auto mb-8 font-light">{{ $store->description }}
                </p>

                <div
                    class="inline-flex items-center justify-center gap-2 text-white/80 border border-white/30 px-6 py-2 rounded-full backdrop-blur-sm hover:bg-white/10 transition cursor-default">
    ...
    <main id="product-display" class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        {{-- HEADER KATALOG --}}
        <div
            class="flex flex-col md:flex-row justify-between items-end mb-12 border-b border-gray-200 dark:border-gray-800 pb-6">
            <div>
                <h2 class="text-4xl md:text-5xl font-heading font-bold text-brand-black dark:text-white uppercase">
                    Katalog <span
                        class="text-transparent bg-clip-text bg-gradient-to-r from-brand-red to-brand-orange">Produk</span>
                </h2>
                <p class="mt-2 text-gray-500 dark:text-gray-400">Pilih nutrisi terbaik untuk menunjang latihanmu.</p>
            </div>
            <div class="mt-4 md:mt-0 flex items-baseline">
                <span
                    class="text-6xl font-heading font-bold text-gray-200 dark:text-gray-800">{{ str_pad($totalProducts, 2, '0', STR_PAD_LEFT) }}</span>
                <span class="text-sm font-bold uppercase tracking-widest text-gray-400 ml-1">Items</span>
            </div>
        </div>
                ...
                <div class="flex flex-col md:flex-row gap-3 items-stretch md:items-center">
                    ...
                    <div class="relative w-full md:flex-grow group">
                        <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                            <i
                                class="fas fa-search text-gray-400 group-focus-within:text-brand-red transition-colors"></i>
                        </div>
                    ...
                    @if (request('search') || request('category'))
                        <a href="{{ route('store.biin-king') }}#product-display"
                            ...
                            <span class="ml-2 md:hidden">Reset Filter</span>
                        </a>
                    @endif
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
            @foreach ($products as $product)
                @php
                    ...
                    $storeName = $product->stores_id == 2 ? 'ISTANA MERDEKA 03' : 'B11N & K1NG Store';
                @endphp

                <div
                    class="group relative bg-white dark:bg-brand-gray rounded-xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-300 border border-gray-100 dark:border-gray-800 flex flex-col h-full">

                    <div class="relative w-full pt-[100%] overflow-hidden bg-gray-100 dark:bg-gray-900">
                        {{-- Optimasi: loading="lazy" pada gambar yang ada di bawah --}}
                        ...
                        <div
                            class="absolute top-3 right-3 bg-black/70 backdrop-blur-md text-white text-[10px] font-bold px-2 py-1 rounded uppercase tracking-wider">
                            {{ $storeName }}
                        </div>
                    </div>
                    ...
                    <div class="p-5 flex flex-col flex-grow">
                        ...
                        <h3
                            class="text-lg font-heading font-bold text-gray-900 dark:text-white leading-tight mb-2 group-hover:{{ $accentColor }} transition-colors">
                            {{ $product->name }}
                        </h3>
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            if (toast) {
                setTimeout(() => {
                    ...
                    setTimeout(() => toast.remove(), 500); // Menunggu transisi selesai
